...
SEXP cdfit_cox_dh(SEXP X_, SEXP d_, SEXP penalty_, SEXP lambda, SEXP eps_, SEXP max_iter_, SEXP gamma_, SEXP multiplier, SEXP alpha_, SEXP dfmax_, SEXP user_, SEXP warn_,SEXP weights_) {
  ...
  double xwr, xwx, u, v, l1, l2, shift, si, s, nullDev, dwr;
  int lstart;

  // If lam[0]=lam_max, skip lam[0] -- closed form sol'n available
  rsk[n-1] = weights[n-1];
  //change 1
  for (int i=n-2; i>=0; i--) rsk[i] = rsk[i+1] + weights[i];
  nullDev = 0;
  for (int i=0; i<n; i++) nullDev -= d[i]*weights[i]*log(rsk[i]);
  if (user) {
    lstart = 0;
  } else {
    lstart = 1;
  ...
  for (int l=lstart; l<L; l++) {
    ...
    while (tot_iter < max_iter) {
      while (tot_iter < max_iter) {
		...
		//change 2
        rsk[n-1] = weights[n-1]*haz[n-1];
        for (int i=n-2; i>=0; i--) {
          rsk[i] = rsk[i+1] + weights[i]*haz[i];
        }
        for (int i=0; i<n; i++) {
			//change 4
          REAL(Loss)[l] += weights[i]*d[i]*eta[i] - d[i]*weights[i]*log(rsk[i]);
        }

        // Approximate L

        h[0] = d[0]*weights[0]/rsk[0];
        for (int i=1; i<n; i++) {
          h[i] = h[i-1] + d[i]*weights[i]/rsk[i];
        }
        for (int i=0; i<n; i++) {
          h[i] = h[i]*haz[i]*weights[i];
          s = d[i]*weights[i] - h[i];
          if (h[i]==0) r[i]=0;
          else r[i] = s/h[i];
        }
